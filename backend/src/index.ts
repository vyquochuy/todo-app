...
async function seedTestUser(db: any) {
  if (isSeeded) return;
  try {
    ...
    if (userCount.total === 0) {
      ...
      console.log("Successfully seeded default test user (test@example.com)");
    }
    isSeeded = true;
  } catch (err) {
...
app.use("/todos/*", async (c, next) => {
  const secret = c.env.JWT_SECRET ?? "fallback-jwt-secret-key-9999";
